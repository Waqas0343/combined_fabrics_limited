...
class CheckOperatorProductionReportController extends GetxController {
  ...
  final TextEditingController selectedWorkOrderController =
  ...
  Future<void> getRowingQualityOperatorProductionReport({
    int? Operator,
    String? fromDate,
    String? todate,
  }) async {
    isLoading(true);
    String data =
        "optr_code=$Operator&Fromdate=$fromDate&Todate=${toDateController.text}&WorkOrder=&lineNo=";
    operatorProductionList.clear();
    List<RowingQualityCheckOperatorProductionReportListModel>? responseList =
        await ApiFetch.getRowingQualityOperatorProductionReport(data);
    isLoading(false);
  ...
  Future<Uint8List> generatePdf(PdfPageFormat format,
      List<RowingQualityCheckOperatorProductionReportListModel> data) async {
  ...
  void sortData(String property, bool ascending) {
    ...
    operatorProductionList.sort((a, b) {
      switch (property) {
        ...
        case 'producepcs':
          return ascending
              ? a.producepcs.compareTo(b.producepcs)
              : b.producepcs.compareTo(a.producepcs);
        case 'ageInMinutes':
          return ascending
              ? a.ageInMinutes.compareTo(b.ageInMinutes)
              : b.ageInMinutes.compareTo(a.ageInMinutes);
        case 'flagShortName':
          return ascending
              ? a.flagShortName.compareTo(b.flagShortName)
              : b.flagShortName.compareTo(a.flagShortName);
        case 'enlineDhu':
          return ascending
              ? a.enlineDhu.compareTo(b.enlineDhu)
              : b.enlineDhu.compareTo(a.enlineDhu);
        default:
          return 0;
      }
    });
